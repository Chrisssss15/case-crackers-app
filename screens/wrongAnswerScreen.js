...
// Importeren van achtergrond, logo en hartjes
...
// Scherm dat wordt getoond als een team een fout antwoord geeft
export default function WrongAnswerScreen({ setCurrentScreen, team }) {
  // Breedte en hoogte van het scherm ophalen
  ...
  const lives = Number(currentTeam.lives ?? 3); // Aantal levens van het team
  const isSmallScreen = height < 750; // Checkt of het scherm kleiner is dan 750px

  // Logo grootte aanpassen aan schermgrootte
  const logoWidth = width * 0.52;
  const logoHeight = logoWidth * 0.2;

  // Hartjes grootte aanpassen aan schermgrootte
...
// Component voor een gevuld of gebroken hartje
